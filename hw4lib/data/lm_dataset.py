...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        # Check if root already includes hw4p1_data
        root = config['root']
        if os.path.basename(root.rstrip('/')) == 'hw4p1_data':
            base_path = root
        else:
            # Root is parent directory, need to add hw4p1_data
            base_path = os.path.join(root, 'hw4p1_data')
        
        # Text files are directly in the partition directory (not in a 'text' subdirectory)
        self.text_dir = os.path.join(base_path, partition)
        ...
        for file in tqdm(self.text_files):
            ...
            text_path = os.path.join(self.text_dir, file)
